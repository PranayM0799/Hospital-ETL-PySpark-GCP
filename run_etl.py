...
def create_spark_session():
    ...
    spark = (
        SparkSession.builder.appName("HospitalETLPipeline")
...
def transform_patient_data(df):
    ...
    df_transformed = (
        df.withColumn("date_of_birth", to_date(col("date_of_birth"), "yyyy-MM-dd"))
        .withColumn(
            "admission_date", to_timestamp(col("admission_date"), "yyyy-MM-dd HH:mm:ss")
        )
        .withColumn(
            "discharge_date", to_timestamp(col("discharge_date"), "yyyy-MM-dd HH:mm:ss")
        )
        .withColumn(
            "created_at", to_timestamp(col("created_at"), "yyyy-MM-dd HH:mm:ss")
        )
    )
    ...
    df_transformed = (
        df_transformed.withColumn(
            "age_at_admission", year(col("admission_date")) - year(col("date_of_birth"))
        )
        .withColumn(
            "length_of_stay_days",
    ...
    df_transformed = df_transformed.withColumn(
        "has_valid_dates",
        when(
            col("admission_date").isNotNull() & col("discharge_date").isNotNull(), True
        ).otherwise(False),
    ).withColumn(
        "has_valid_diagnosis",
        when(col("diagnosis").isNotNull() & (col("diagnosis") != ""), True).otherwise(
            False
        ),
    )
...
def transform_treatment_data(df):
    ...
    df_transformed = (
        df.withColumn(
            "treatment_date", to_timestamp(col("treatment_date"), "yyyy-MM-dd HH:mm:ss")
        )
        .withColumn(
            "created_at", to_timestamp(col("created_at"), "yyyy-MM-dd HH:mm:ss")
        )
    ...
    # Add calculated fields
    df_transformed = (
        df_transformed.withColumn("treatment_year", year(col("treatment_date")))
        .withColumn("treatment_month", month(col("treatment_date")))
        .withColumn("treatment_day_of_week", dayofweek(col("treatment_date")))
        .withColumn(
            "cost_category",
    ...
    df_transformed = (
        df_transformed.withColumn(
            "has_valid_cost",
            when(col("cost").isNotNull() & (col("cost") >= 0), True).otherwise(False),
        )
        .withColumn(
            "has_valid_doctor",
            when(
                col("doctor_name").isNotNull() & (col("doctor_name") != ""), True
            ).otherwise(False),
        )
        .withColumn(
            "has_valid_treatment_type",
            when(
                col("treatment_type").isNotNull() & (col("treatment_type") != ""), True
            ).otherwise(False),
        )
    )
...
def transform_hospital_analysis_data(df):
    ...
    df_transformed = df_transformed.withColumn(
        "outcome", when(col("outcome").isNull(), "Unknown").otherwise(col("outcome"))
    )
...
def validate_data(df, data_type):
    ...
    if data_type == "patient":
        ...
        null_names = df.filter(
            col("first_name").isNull() | col("last_name").isNull()
        ).count()
        null_dates = df.filter(
            col("admission_date").isNull() | col("discharge_date").isNull()
        ).count()
        ...
        duplicate_patient_ids = (
            df.groupBy("patient_id").count().filter(col("count") > 1).count()
        )
        ...
        duplicate_treatment_ids = (
            df.groupBy("treatment_id").count().filter(col("count") > 1).count()
        )
...
def write_to_bigquery(df, project_id, dataset_id, table_id):
    ...
    try:
        ...
        df_to_write.write.format("bigquery").option(
            "table", f"{project_id}.{dataset_id}.{table_id}"
        ).option("writeMethod", "indirect").option(
            "temporaryGcsBucket", "hospital-processed-data-1ed88d11"
        ).option(
...
def main():
    ...
    parser.add_argument(
        "--dataset-id", default="hospital_data", help="BigQuery dataset ID"
    )
    ...
    try:
        ...
        write_to_bigquery(
            patient_transformed, args.project_id, args.dataset_id, "patients"
        )
        ...
        write_to_bigquery(
            treatment_transformed, args.project_id, args.dataset_id, "treatments"
        )
        ...
        write_to_bigquery(
            analysis_transformed, args.project_id, args.dataset_id, "hospital_analysis"
        )
