...
def transform_patient_data(df):
    ...
    df_transformed = df_transformed.withColumn(
        "has_valid_dates",
        when(col("admission_date").isNotNull() & col("discharge_date").isNotNull(), True)
        .otherwise(False)
    ).withColumn(
        "has_valid_diagnosis",
        when(col("diagnosis").isNotNull() & (col("diagnosis") != ""), True)
        .otherwise(False)
    )
...
def transform_treatment_data(df):
    ...
    df_transformed = df_transformed.withColumn(
        "has_valid_cost",
        when(col("cost").isNotNull() & (col("cost") >= 0), True)
        .otherwise(False)
    ).withColumn(
        "has_valid_doctor",
        when(col("doctor_name").isNotNull() & (col("doctor_name") != ""), True)
        .otherwise(False)
    ).withColumn(
        "has_valid_treatment_type",
        when(col("treatment_type").isNotNull() & (col("treatment_type") != ""), True)
        .otherwise(False)
    )
...
def transform_hospital_analysis_data(df):
    ...
    df_transformed = df_transformed.withColumn("gender", 
        when(col("gender").isNull(), "Unknown").otherwise(col("gender")))
    
    df_transformed = df_transformed.withColumn("condition", 
        when(col("condition").isNull(), "Unknown").otherwise(col("condition")))
    
    df_transformed = df_transformed.withColumn("procedure", 
        when(col("procedure").isNull(), "Unknown").otherwise(col("procedure")))
    
    df_transformed = df_transformed.withColumn("outcome", 
        when(col("outcome").isNull(), "Unknown").otherwise(col("outcome")))
    
    df_transformed = df_transformed.withColumn("readmission", 
        when(col("readmission").isNull(), "No").otherwise(col("readmission")))
    
    # Ensure satisfaction is between 1-5
    df_transformed = df_transformed.withColumn("satisfaction", 
    ...
    df_transformed = df_transformed.withColumn("length_of_stay", 
        when(col("length_of_stay").isNull(), 0)
        .when(col("length_of_stay") < 0, 0)
...
def validate_data(df, data_type):
    ...
    if data_type == "patient":
        ...
        null_names = df.filter(col("first_name").isNull() | col("last_name").isNull()).count()
        null_dates = df.filter(col("admission_date").isNull() | col("discharge_date").isNull()).count()
        ...
        duplicate_patient_ids = df.groupBy("patient_id").count().filter(col("count") > 1).count()
        ...
        null_patient_ids = df.filter(col("patient_id").isNull()).count()
        null_treatment_types = df.filter(col("treatment_type").isNull()).count()
        null_costs = df.filter(col("cost").isNull()).count()
        
        logger.info(f"Records with null treatment_id: {null_treatment_ids}")
        logger.info(f"Records with null patient_id: {null_patient_ids}")
        logger.info(f"Records with null treatment_type: {null_treatment_types}")
        logger.info(f"Records with null cost: {null_costs}")
        
        # Check for duplicate treatment IDs
        duplicate_treatment_ids = df.groupBy("treatment_id").count().filter(col("count") > 1).count()
...
def write_to_bigquery(df, project_id, dataset_id, table_id):
    ...
    logger.info(f"Writing data to BigQuery: {project_id}.{dataset_id}.{table_id}")
    ...
    try:
        ...
        if table_id == "patients":
            ...
            df_to_write = df.select(
                ...
                "gender", "admission_date", "discharge_date", "diagnosis", "created_at"
            )
        elif table_id == "treatments":
            # Select only the original columns for treatments table
            df_to_write = df.select(
                "treatment_id", "patient_id", "treatment_type", "treatment_date",
                "doctor_name", "treatment_notes", "cost", "created_at"
            )
            ...
            df_to_write = df.select(
                ...
                "cost", "length_of_stay", "readmission", "outcome", "satisfaction", "created_at"
            )
        else:
            df_to_write = df
...
def main():
    ...
    parser.add_argument('--raw-bucket', required=True, help='Raw data bucket name')
    parser.add_argument('--dataset-id', default='hospital_data', help='BigQuery dataset ID')
    ...
    try:
        ...
        patient_df = read_patient_data(spark, f"gs://{args.raw_bucket}/data/raw/patients.csv")
        ...
        write_to_bigquery(patient_transformed, args.project_id, args.dataset_id, "patients")
        
        # Process treatment data
        logger.info("Processing treatment data...")
        treatment_df = read_treatment_data(spark, f"gs://{args.raw_bucket}/data/raw/treatments.csv")
        ...
        write_to_bigquery(treatment_transformed, args.project_id, args.dataset_id, "treatments")
        
        # Process hospital analysis data
        logger.info("Processing hospital analysis data...")
        analysis_df = read_hospital_analysis_data(spark, f"gs://{args.raw_bucket}/data/raw/hospital data analysis.csv")
        ...
        write_to_bigquery(analysis_transformed, args.project_id, args.dataset_id, "hospital_analysis")
        ...
        logger.info(f"- Data loaded to BigQuery: {args.project_id}.{args.dataset_id}")
